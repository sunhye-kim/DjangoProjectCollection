...
class CafeLV(APIView):
    ...
    def post(self, request):
        ...
        cafe_main_serializer = serializers.CafeSerializers(data=data)
        ...
        if data.get("address"):
            cafe_address_data = data.get("address")
        ...
        return Response(cafe_main_serializer.data, status=status.HTTP_201_CREATED)
...
class CafeMenuLV(APIView):
    def get(self, request, cafe_id):
        cafe_lv = get_object_or_404(CafeMain, cafe_no=cafe_id)
...
class CafeMenuDV(APIView):
    def get(self, request, cafe_id, menuname):
        cafe_dv = get_object_or_404(CafeMain, cafe_no=cafe_id, menu_name=menuname)
